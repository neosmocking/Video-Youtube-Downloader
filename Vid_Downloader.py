...
def download_video():
    """Fungsi download menggunakan yt-dlp dan menyimpan ke folder 'Downloads' lokal."""
    link = input("Masukkan link video YouTube: ")

    # --- PERUBAHAN DIMULAI DI SINI ---
    # Tentukan path ke folder tempat script berada
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Buat path untuk folder 'Downloads' di dalam folder script
    download_folder = os.path.join(script_dir, 'Downloads')

    # Buat folder 'Downloads' jika belum ada
    os.makedirs(download_folder, exist_ok=True)
    
    # Siapkan path lengkap untuk nama file output
    ...
    ydl_opts = {
        ...
        # Nama file output: disimpan di folder Downloads lokal
        'outtmpl': output_path,
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # --- Tambahkan info lokasi penyimpanan ---
            print(f"\nVideo akan disimpan di folder lokal: {download_folder}")
            print("\n--- Memulai Download... ---")
